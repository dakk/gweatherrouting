...
class GribRecord:
	def __init__ (self, f):
		...
		if flagoptionalsez[0]=="1":
			...
			if DRType != 0:
				...
				data=f.read(1)#bit 17
				self.flaguvcomp=str8bit(data)#stringa di 8bit
				...
				#self.bounds = [(LA1, LO1), (LA1 + NI * DI, LO1 + NJ * DJ)]

				data=f.read(1)
				self.flagscanmode=str8bit(data)#stringa di 8bit
		...
		if flagdata[0]=="0" and flagdata[1]=="0" and flagdata[2]=="0" and flagdata[3]=="0":
			...
			for i in range(0,self.size[1]):
				self.grid.append([0] * self.size[0])
			bufferstr=""
			i,j = self.initij ()
			...
			for N in range(0,NI*NJ):
				...
				#print (j, i, len (self.grid[0]))
				self.grid[j][i]=(R+X*2**E)*10**-D
				i,j = self.incij (i,j)
	...
	def initij (self):
		if self.flagscanmode[0]=="0":
			i = 0
		else:
			i = self.size[0]-1
		if self.flagscanmode[1]=="1":
			j = 0
		else:
			j = self.size[1]-1
		return i,j
	...
	def getValue2 (self, lat, lon):
		...
		i=int ((lon-minlon)/(maxlon-minlon))
		j=int ((lat-minlat)/(maxlat-minlat))
	...
	def getValue (self,lat,lon):
		...
		if (lat>=minlat and lat<maxlat) and (lon>=minlon and lon<maxlon):
			i=(lon-minlon)/(maxlon-minlon)*self.size[0]
			j=(lat-minlat)/(maxlat-minlat)*self.size[1]
			di=i-int(i)#parte decimale
			dj=j-int(j)#parte decimale
			riga=int(j)
			colonna=int(i)
			#interpolazione primo ordine
			valore11=self.grid[riga][colonna]
			valore12=self.grid[riga][colonna+1]
			valore13=self.grid[riga+1][colonna]                    
			valore1=valore11+(valore12-valore11)+dj*1.0/self.dirincrement[1]+(valore13-valore11)*di*1.0/self.dirincrement[0]
			#per u e v
			if self.flaguvcomp=="1":#componenti concordi con la griglia
				if parameter==34:valore1=valore1*math.copysign(1,self.dirincrement[1])#rese northerly
				if parameter==33:valore1=valore1*math.copysign(1,self.dirincrement[0])#rese easterly
		#else:
		#	print ('fail')
		return valore1
...
class Grib:
	...
	def getWind (self, t, lat, lon):
		...
		tws=0
		twd=0
		u=self.rindex[33][t].getValue (lat,lon)
		v=self.rindex[34][t].getValue (lat,lon)
		if u!="" and v!="":
				tws=(u**2+v**2)**0.5
